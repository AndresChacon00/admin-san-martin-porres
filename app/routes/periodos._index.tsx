...
export const action: ActionFunction = async ({ request }) => {
  try {
    ...
    if (actionType === 'agregar') {
      ...
      try {
        ...
        if (isNaN(d1.getTime()) || isNaN(d2.getTime())) {
          if (wantsJson)
            return json(
              { type: 'error', message: 'Fechas inválidas' },
              { status: 400 },
            );
      ...
      if (
        ...
        const msg = result.message || 'Error al añadir un periodo';
        if (wantsJson)
          return json({ type: 'error', message: msg }, { status: 400 });
        return { error: msg };
      }

      if (wantsJson)
        return json({ type: 'success', message: 'Periodo agregado' });
      return null;
    }
    ...
    if (actionType === 'editar') {
      ...
      if (wantsJson)
        return json({ type: 'success', message: 'Periodo actualizado' });
      return null;
    }
    ...
    if (actionType === 'eliminar') {
      ...
      if (typeof idPeriodoRaw !== 'string' || idPeriodoRaw.trim() === '') {
        if (wantsJson)
          return json(
            { type: 'error', message: 'ID inválido' },
            { status: 400 },
          );
      ...
      if (wantsJson)
        return json({ type: 'success', message: 'Periodo eliminado' });
      return null;
    }

    if (wantsJson)
      return json(
        { type: 'error', message: 'Acción no válida' },
        { status: 400 },
      );
                  ...
                  if (res.status < 400) {
                    ...
                    if (isHtml) {
                      ...
                      if (specificMatch) {
                        ...
                        const uniqueMatch =
                          text.match(/UNIQUE constraint failed:\s*([\w.]+)/i) ||
                          text.match(
                            /UNIQUE constraint failed\s*:\s*([\w.\s,]+)/i,
                          );
                        if (uniqueMatch) {
                          const full = uniqueMatch[1];
                          const idMatch = full.match(
                            /(?:\.|_)(idPeriodo|id_periodo|id-periodo)/i,
                          );
                          msg = idMatch
                            ? `El id del periodo ya está registrado`
                            : 'Error de constraint en la base de datos (valor duplicado)';
                        } else {
                          msg =
                            'Error agregando periodo (ver consola para detalles)';
                        }
                      }
                      // Log full HTML to console for debugging
                      // eslint-disable-next-line no-console
                      console.error(
                        'Agregar Periodo HTML response (truncated):',
                        text.slice(0, 2000),
                      );
                    } else {
                      msg = msg || text || 'Error agregando periodo';
                      // eslint-disable-next-line no-console
                      console.debug('Agregar Periodo error response', {
                        status: res.status,
                        parsed,
                        headerMsg,
                        text,
