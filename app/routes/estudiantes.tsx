...
export const action: ActionFunction = async ({ request }) => {
  try {
    ...
    if (actionType === 'agregar') {
      ...
      if (wantsJson)
        return json({ type: 'success', message: 'Estudiante agregado' });
    }
    ...
    if (actionType === 'editar') {
      ...
      if (wantsJson)
        return json({ type: 'success', message: 'Estudiante actualizado' });
    }
    ...
    if (actionType === 'eliminar') {
      ...
      if (typeof cedula !== 'string') {
        return json(
          { type: 'error', message: 'Cedula inválida' },
          { status: 400 },
        );
      }
      ...
      if (wantsJson)
        return json({ type: 'success', message: 'Estudiante eliminado' });
    }
...
export default function EstudiantesPage() {
  ...
  const formattedData = useMemo(() => {
    ...
    return data.map((estudiante) => ({
      ...estudiante,
      edad: estudiante.fechaNacimiento
  ...
  return (
    <>
      ...
      <div className='py-4 w-3/4'>
        ...
        <main className='py-4'>
          {'type' in formattedData && formattedData.type === 'error' && (
            <p>Ocurrió un error cargando los datos</p>
          )}
          {!('type' in formattedData) && (
