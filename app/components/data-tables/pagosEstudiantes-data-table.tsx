...
export function DataTablePagosEstudiantes({
  columns,
  data,
  onGenerateReceipt,
}: DataTableProps) {
  ...
  const [action, setAction] = useState<
    'edit' | 'delete' | 'generateReceipt' | null
  >(null);
    ...
    <div className='rounded-md border'>
      <div className='flex items-center py-4 pl-4'>
        <Input
          ...
          value={
            (table.getColumn('cedulaEstudiante')?.getFilterValue() as string) ??
            ''
          }
          onChange={(event) =>
            table
              .getColumn('cedulaEstudiante')
