...
interface ReceiptProps {
  ...
  telefono: string;
  correo: string;
  direccionPuntoReferencia: string;
  ...
  logoSrc?: string;
  nombreCentroCapacitacion?: string;
}
...
const Receipt: React.FC<ReceiptProps> = ({
  ...
  observaciones,
  logoSrc,
  nombreCentroCapacitacion,
}) => {
  const currentDate = new Date().toLocaleDateString();

  return (
    <div style={styles.receiptContainer} id="receipt-to-print">
      {logoSrc && <div style={styles.logoContainer}><img src={logoSrc} alt="Logo Curso" style={styles.logo} /></div>}
      {nombreCentroCapacitacion && <h3 style={styles.centerText}>{nombreCentroCapacitacion}</h3>}
      ...
      <div style={styles.row}>
        ...
        <div style={styles.column}>Talla: {talla}</div>
      </div>
      ...
      <div style={styles.row}>
        <div style={styles.column}>EDAD: {edad}</div>
      ...
      <div style={styles.fullWidth}>DIRECCION Y PUNTO DE REFERENCIA: {direccionPuntoReferencia}</div>
